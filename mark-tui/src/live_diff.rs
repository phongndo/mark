...
use mark_diff::{Changeset, DiffOptions, DiffSource};
    ...
    runtime::spawn(async move {
        ...
        while let Some(LiveDiffCommand::Changed) = control_rx.recv().await {
            ...
            .await
            {
                ...
                Err(error) => Err(MarkError::Io(std::io::Error::other(format!(
...
pub(crate) fn watcher_error(context: &str, error: notify::Error) -> MarkError {
    MarkError::Usage(format!("{context}: {error}"))
}
