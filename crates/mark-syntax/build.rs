...
fn main() {
    ...
    println!(
        "cargo:rustc-env=MARK_SYNTAX_BUNDLE_PATH={}",
        output.display()
    );
...
fn build_bundle(assets: &Path, input_hash: u64) -> Result<Vec<u8>, String> {
    ...
    let license_by_language = licenses
        ...
        .filter_map(|asset| {
            Some((
                asset.get("language")?.as_str()?.to_owned(),
                (
                    asset
                        .get("path")
                        .and_then(|v| v.as_str())
    ...
    for grammar in &grammars {
        let (source_path, spdx_id) = license_by_language
            .get(&grammar.language)
            .cloned()
            .unwrap_or_else(|| (grammar.path.clone(), default_license.clone()));
    ...
    match value {
        serde_json::Value::Object(map) => {
            for (key, value) in map {
                if (key == "name" || key == "contentName") && value.as_str().is_some() {
