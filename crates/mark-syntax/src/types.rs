...
pub struct SyntaxHighlighter {
    tokenizers: BTreeMap<String, syntaxmate::Tokenizer>,
...
impl SyntaxHighlighter {
    ...
    pub fn highlight(&mut self, language: &str, source: &str) -> MarkResult<HighlightedText> {
        ...
        if !self.tokenizers.contains_key(&canonical) {
            ...
            let tokenizer = syntaxmate::Tokenizer::for_bundled_language(
                &canonical,
                syntaxmate::TokenizerOptions {
                    max_line_bytes: limits.max_line_bytes,
                    line_cache_entries: limits.engine_line_cache_entries(),
                },
            )
            ...
            let tokenizer = {
                ...
                tokenizer.set_diagnostics_enabled(self.counters_enabled);
                tokenizer
            };
    ...
    pub fn set_engine_counters_enabled(&mut self, enabled: bool) {
        ...
        for tokenizer in self.tokenizers.values_mut() {
            tokenizer.set_diagnostics_enabled(enabled);
        }
    }

    #[cfg(feature = "diagnostics")]
    pub fn take_engine_counters(&mut self) -> syntaxmate::diagnostics::EngineCounters {
        let mut counters = syntaxmate::diagnostics::EngineCounters::default();
        for tokenizer in self.tokenizers.values_mut() {
            counters.merge(tokenizer.take_diagnostics());
        }
        counters
    }
}

fn adapt_highlighted_text(
    source: &str,
