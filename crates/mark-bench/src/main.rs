...
enum BenchCommand {
    ...
    #[command(about = "Measure patch loading and TUI rendering")]
    Measure(MeasureArgs),
...
struct MeasureArgs {
    ...
    #[arg(long)]
    syntax: bool,
    /// Language to request for the syntax run. Repeat to request several languages.
...
impl ScenarioKind {
    ...
    fn description(self) -> &'static str {
        match self {
            ...
            Self::SyntaxManySmallRust => "Rust many-small-file diff for syntax-oriented runs.",
            Self::SyntaxLargeRust => "Rust large-single-file diff for syntax-oriented runs.",
...
fn measure_fixtures(args: MeasureArgs) -> BenchResult<()> {
    ...
    ensure_syntax_benchmark_ready(&syntax_languages)?;
...
fn ensure_syntax_benchmark_ready(languages: &[String]) -> BenchResult<()> {
    if languages.is_empty()
        || !mark_syntax::SyntaxLanguageSet::from_enabled_languages(languages).is_empty()
    {
        return Ok(());
    }

    Err(BenchError::Usage(
        "syntax benchmark requested, but no requested language is highlight-ready".to_owned(),
    ))
}
...
mod tests {
    ...
    #[test]
    fn syntax_benchmark_rejects_languages_without_a_ready_backend() {
        assert!(ensure_syntax_benchmark_ready(&[]).is_ok());

        let error = ensure_syntax_benchmark_ready(&["rust".to_owned()])
            .unwrap_err()
            .to_string();

        assert!(error.contains("no requested language is highlight-ready"));
    }
