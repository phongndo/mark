...
Status: the independent `syntaxmate 0.1.0` crate is published on crates.io.
Mark's thin adapter resolves that registry release through `Cargo.lock`, and the
complete downstream Rust, MSRV, theme, performance, packaging, and distribution
regression suites pass without a path, Git, or patch override.
